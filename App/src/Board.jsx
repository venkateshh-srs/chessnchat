...
if (storedName === null) {
  ...
  socket.emit("checkReload", {
    type: sessionStorage.getItem("selectedColor"),
...
function EmptyBoard() {
  ...
  const handleWhiteClickButton = () => {
    ...
    sessionStorage.setItem("selectedColor", "w");
  ...
  const handleBlackClickButton = () => {
    ...
    sessionStorage.setItem("isUserTakenBlackOrWhite", true);
    sessionStorage.setItem("selectedColor", "b");
  ...
  const handle = (i, j, type) /** target*/ => {
    ...
    if (source.i !== null && source.j !== null) {
      if (source.type.charAt(2) !== sessionStorage.getItem("selectedColor")) {
        source = { i: i, j: j, type: type };
        return false;
      }
    }
  ...
  let Board = () => {
    return (
      <>
        ...
        {rotateBoardto === "b" ||
        sessionStorage.getItem("selectedColor") === "b"
          ? balcksView
          : whitesView}
      </>
    );
